...
def cal_distances(cluster_manager: ClusterManager,reference=1e-8,eps=0.1, max_iter=1000):
    ...
    target_cluster_coordinates = target_files.parallel_applymap(lambda x: pd.read_csv(x, index_col=0))
    pandarallel.initialize()
    target_atoms = target_cluster_coordinates.parallel_applymap(lambda x: set(x.loc[:, 'atom'].unique().tolist()))
    target_atoms = target_atoms.parallel_apply(lambda x: set.union(*x.values.tolist()), axis=1)
    ...
    for pickup_atom in target_atoms:
        pandarallel.initialize()
        pickup_coordinates = target_cluster_coordinates.parallel_applymap(lambda x: x.loc[x.loc[:, 'atom'] == pickup_atom, ['x', 'y', 'z']].to_numpy().tolist())
