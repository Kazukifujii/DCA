...
def make_sort_distance(selfcsv,csvn='sort_distanc.csv'):
    ...
    standdf.to_csv(csvn)
    indexlist=list(set(standdf.isite_i.to_list()+standdf.isite_j.to_list()))
    matrixdf=pd.DataFrame(index=indexlist,columns=indexlist)
    for i,data in standdf.iterrows():
        matrixdf.loc[data.isite_i,data.isite_j]=deepcopy(data.distance)
        matrixdf.loc[data.isite_j,data.isite_i]=deepcopy(data.distance)
    ...
    if len(os.path.dirname(csvn))==0:
        #print('matrix_{}'.format(os.path.basename(csvn)))
        matrixdf.to_csv('matrix_{}'.format(os.path.basename(csvn)))
    else:
        #print('{}/matrix_{}'.format(os.path.dirname(csvn),os.path.basename(csvn)))
